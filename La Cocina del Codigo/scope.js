...
Cuando hacemos referencia a una var , js comienza buscando en el entorno mas cercano, ej dentro de bloque if y seguira buscando en entornos mas lejanos hasta que encuentre la variable declarada
...
?Como? 
*Si usamos var, let o const
?Donde? 
*De manera libre fuera de toda F o bloque de codigo, o dentro de una funcion, un if , etc, pero solo con let o const

!Js tiene un SCOPE LEXICO(LEXICAL SCOPING): 
*El scope de cada var se determina leyendo el codigo del programa sin ejecutarlo, se lo conoce tambien como SCOPE ESTITICO
!SCOPE VS CONTEXTO 
...
*Ej dos funciones distintas con var iguales:
var fruta = 'banana' o var fruta = 'manzana'. 
*Por mas que las var sean iguales, estan en contextos distintos 

!CONTEXTO VS CONTEXTO DE EJECUCION (SON COSAS DISTINTAS)

*1) CONTEXTO: el VALOR que tiene la var this en algun momento de la ejecucion. Es decir, CUAL ES EL OBJETO QUE ESTÁ EJECUTANDO UNA FUNCION. 
...
!A) SCOPE GLOBAL: las var globales pueden ser accedidas desde cualquier lugar de nuestro programa. IMPORTANTE: 
...
*Las var globales, permanecen en memoria durante toda la ejecucion del programa 
...
*-Variable declaradas con var siempre van a tener scope de F, sin importar en qué parte de la funcion las declaremos (VARIAB C/ VAR = SCOPE DE F)
...
*Si dentro de un bloque declaramos una var con let o const, fuera de ese bloque no vamos a poder accederla (ver ej) (VARIABLES C/LET O CONST = SCOPE DE BLOQUE)
