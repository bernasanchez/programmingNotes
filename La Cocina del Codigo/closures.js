...
!En Js podmeos asignar una F a una var, es decir, tratar a las F como cualquier otro valor . 
?Como lo podemos hacer? Ver parte 2 de la F
...
function crearContador(){
    ...
    setTimeout(function (){ //*creamos un nuevo closure 
        contador = 100 //*Variable "interna", crea el vinculo con scope sup 
    }, 5000)
...
!A)FASE DE CREACION: 
*Donde se carga en memoria todo lo necesario para ejecutar esa funcion. Ej. 
*1)archivo al q pertenece la F, 
*2)puntero de la prox linea a ejecutar(:1), 
*3)se crea el obj global(window), 
*4)se pone el valor que tiene this dentro de la F(wddow), 
*5)se asocia el contexto de ej con el codigo q se va a ejecutar(todo nuestro script), 
*6)se crea un ENTORNO LEXICO: es un objeto q tienen los Context de Ejec donde se almacenan los NOMBRES DE LAS VAR que existen dentro de una F y los VALORES ACTUALES que tienen (modo clave valor. ej nombre = 'sacha'). Si le asignamos un nuevo valor, cambian el entonrno lexico.
!Dento de este entorno, las claves son unicas, no podemOs tener dos var con el mismo nombre dentro de la misma F (REGISTRO DE ENTORNO/ENVIROMENT RECORD). Ademas, cada entorno lex tiene un "puntero ext:" a su entorno lex exterior (el entorno en el q éste fue creado (el entorno))

*En nuestro caso (funcion de arriba): se crea un ELex(scope) y Js ya sabe que cuenta 3 identificadores de variables: 
*1)Nombre de la F:  crearContador = fn. Como es una F declarada se carga compleamente en memoria cuando se esta creando este CE y cada vez q Js crea una nueva F, la vincula al entorno que la creo, asi esta funcion queda vinculada al EGlob
...
*En el ej anterior, la unica manera de acceder a la var contador es incrementandolo, cuando lo hagas, te voy a devolver el valor incrementado (return), pero no vas a poder acceder al valor directo del contador: 
*contador1.contador. 
...
!API DE UN OBJETO: 
Nos permiten interactuar con los obj. Que metodos podemos invocar sobre ese objeto y que prop internas nos permite acceder
