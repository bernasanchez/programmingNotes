...
De esta manera, cuando terminen de ejecutar la función con la que están trabajando saben cuál es la función con la que deben continuar  después.
...
FUNDAMENTAL: el motor puede ejecutar una sola cosa a la vez. La funcion asociada al registro que esta arriba de todo emn la pila
cuando termina de ejecutarla , la saca de la pila y continua con la otra funcion que sigue en la pila
...
CONTEXTO DE EJECUCION(SCOPE): contexto actual de ejecucion. Existe el Local y el GlobalEste contexto de ejecucion esta determinado por el conjunto de variables y funciones que se pueden acceder cuando se esta ejecutando una funcion.
El conjunto de variables que tiene acceso a la F, el contexto(this), arguments u obj global(windows o global)

THIS: objeto "dueño" de la funcion y el valor que tiene this, determina el contexto de la funcion

ARGUMENTS: obj similar a un array que tienen las F en su scope local(menos las F flecha). Contiene todos loa parametros de la F cuando fue invocada
